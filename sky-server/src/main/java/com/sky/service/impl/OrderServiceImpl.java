...
public class OrderServiceImpl implements OrderService {
    ...
    public void userCancelById(Long id) throws Exception {
        ...
        if (ordersDB.getStatus() > TO_BE_CONFIRMED) { // > 2
        ...
        // 订单处于待接单状态, 需要退款
        if (ordersDB.getStatus().equals(TO_BE_CONFIRMED)) { // = 2
            // 调用微信支付接口退款
            weChatPayUtil.refund(
                    ordersDB.getNumber(), // 商户订单号
                    ordersDB.getAddress(), // 商户退款单号
    ...
    public void repetition(Long id) {
        ...
        // 将`订单详情`对象转换为`购物车对象`
        // orderDetailList -> shoppingCartList
        List<ShoppingCart> shoppingCartList = orderDetailList.stream().map(x -> {
            // 购物车对象
            ShoppingCart shoppingCart = new ShoppingCart();

            // 将原订单详情里面的菜品信息复制到购物车对象
            BeanUtils.copyProperties(x, shoppingCart, "id"); // 忽略id字段
            shoppingCart.setUserId(userId);
            shoppingCart.setCreateTime(LocalDateTime.now());

            // 返回购物车对象
            return shoppingCart;
        }).collect(Collectors.toList()); // 收集成购物车列表
    ...
    public PageResult conditionSearch(OrdersPageQueryDTO ordersPageQueryDTO) {
        ...
        // 部分订单状态, 需要额外返回`订单菜品信息`, 将Orders转换成OrderVO
    ...
    public void confirm(OrdersConfirmDTO ordersConfirmDTO) {
        Orders orders = builder().id(ordersConfirmDTO.getId()).status(CONFIRMED).build();
        orderMapper.update(orders);
    }
    ...
    public void delivery(Long id) {
        ...
        // 校验订单是否存在, 且状态=3
        if (orderDB == null && !orderDB.getStatus().equals(CONFIRMED)) {
            throw new OrderBusinessException(ORDER_STATUS_ERROR);
        }
    ...
    public void complete(Long id) {
        ...
        // 校验订单是否存在, 并且状态为4
        if (ordersDB == null && !ordersDB.getStatus().equals(DELIVERY_IN_PROGRESS)) {
